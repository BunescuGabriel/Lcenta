    ...
    try {
      ...
      if (userIsSuperUser && storedAccessToken && selectedCondition) {
        ...
        if (response.status === 200) {
          refreshData(); // Actualizează lista de condiții după modificare
          toggleAddConditionForm(); // Închide formularul de actualizare după terminare
          setSelectedCondition(null); // Resetează condiția selectată
        } else {
  ...
  const updateDescrierea = async () => {
    try {
      ...
      if (userIsSuperUser && storedAccessToken && SelectedDescrierea) {
        ...
        if (response.status === 200) {
          refreshData(); // Actualizează lista de condiții după modificare
          toggleAddDescriptionForm(); // Închide formularul de actualizare după terminare
          setSelectedDescrierea(null); // Resetează condiția selectată
        } else {
          throw new Error('Failed to update condition: Network response was not ok');
        }
      } else {
        ...
        {showAddConditionForm ? (
          <form onSubmit={(e) => e.preventDefault()}>
            ...
            <div>
              ...
              <button onClick={setSelectedCondition ? addCondition : updateCondition}>
              {setSelectedCondition ? 'Actualizează' : 'Adaugă'}
            </button>
